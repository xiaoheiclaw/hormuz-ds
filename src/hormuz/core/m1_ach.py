...
def run_ach(
    ...
    Stateless: computes posterior from the full observation set each time.
    Pass ALL historical observations (one per O-series, latest value) for
    correct cumulative effect. Idempotent — same input → same output.

    1. Start from flat prior (50:50 when H3 suspended)
